...
typedef int SOCKET;
#  endif
#  if defined(__CYGWIN__) || defined(__OpenBSD__) || defined(__FreeBSD__) || defined(__NetBSD__) || \
...
public:
     ...
     */
    enum { READFILE = 0x1, WRITEFILE = 0x2, DELETEFILE = 0x4, LIST = 0x8, CREATEDIR = 0x10, DELETEDIR = 0x20 };
...
private:
    ...
    enum { MaxLoginLen = 16, MaxPasswordLen = 16, MaxRootPathLen = MAX_PATH };
...
private:
    ...
    enum Status_t { WAITING, LISTING, UPLOADING, DOWNLOADING } volatile eStatus;
    ...
    enum { MAX_COMMAND_LINE_LEN = MAX_PATH + 32 };
    ...
    enum DataMode_t { STREAM, ZLIB } eDataMode;
    ...
    enum DataConnection_t { NONE, PASV, PORT } eDataConnection;
...
#  endif
     ...
     */
    int GetFileListLine(char*          psLine,
                        unsigned short mode,
#ifdef __USE_FILE_OFFSET64
                        __int64 size,
#else

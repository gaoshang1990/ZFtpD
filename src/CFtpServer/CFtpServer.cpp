...
char* CFtpServer::CClientEntry::BuildPath(char* pszAskedPath, char** pszVirtualPath /* = NULL */)
{
    ...
    if (pszBuffer) {
        ...
        if (pszVirtualP) {
            ...
            if (snprintf(pszBuffer, MAX_PATH + 4, "%s/%s", pUser->GetStartDirectory(), pszVirtualP) > 0) {
    ...
    if (pszBuffer) {
        ...
        if (pszAskedPath) {
            if (pszAskedPath[0] == '/') {
                ...
                if (snprintf(pszBuffer, MAX_PATH + 4, "%s/%s", GetWorkingDirectory(), pszAskedPath) > 0) {
                    SimplifyPath(pszBuffer);
                    if (strlen(pszBuffer) > MAX_PATH)
                        bErr = true;
                }
...
{
    if (pszLogin && pszStartDir && strlen(pszLogin) <= MaxLoginLen && strlen(pszStartDir) <= MaxRootPathLen &&
    ...
    for (;;) { // Main Loop
        ...
        if (nCmd == CMD_NONE) {
            ...
            if (pClient->eStatus == WAITING && pszCmdArg &&
                ...
                iIp1 <= 255 && iIp2 <= 255 && iIp3 <= 255 && iIp4 <= 255 && iPort1 <= 255 && iPort2 <= 255 &&
...
#ifdef WIN32
                pClient->CurrentTransfer.hTransferThread = (HANDLE)_beginthreadex(
                    NULL, 0, ListThread, pClient, 0, &pClient->CurrentTransfer.uTransferThreadID);
                if (pClient->CurrentTransfer.hTransferThread == 0) {
#else
                if (pthread_create(&pClient->CurrentTransfer.TransferThreadID,
                                   &pFtpServer->m_pattrTransfer,
                                   ListThread,
                                   pClient) != 0)
                {
#endif
...
#ifdef WIN32
                    pClient->CurrentTransfer.hTransferThread = (HANDLE)_beginthreadex(
                        NULL, 0, RetrieveThread, pClient, 0, &pClient->CurrentTransfer.uTransferThreadID);
                    if (pClient->CurrentTransfer.hTransferThread == 0) {
#else
                    if (pthread_create(&pClient->CurrentTransfer.TransferThreadID,
                                       &pFtpServer->m_pattrTransfer,
...
#ifdef WIN32
                    pClient->CurrentTransfer.hTransferThread = (HANDLE)_beginthreadex(
                        NULL, 0, StoreThread, pClient, 0, &pClient->CurrentTransfer.uTransferThreadID);
                    if (pClient->CurrentTransfer.hTransferThread == 0) {
#else
                    if (pthread_create(&pClient->CurrentTransfer.TransferThreadID,
                                       &pFtpServer->m_pattrTransfer,
...
bool CFtpServer::CClientEntry::ReceiveLine()
{
    ...
    for (;;) {
        ...
        if (!FD_ISSET(CtrlSock, &fdRead)) {
            dt = difftime(time(NULL), tTimeoutTime);
            if ((!IsLogged() && pFtpServer->GetNoLoginTimeout()) ||
                (eStatus == WAITING && pFtpServer->GetNoTransferTimeout() && dt >= pFtpServer->GetNoTransferTimeout()))
            {
...
bool CFtpServer::CClientEntry::OpenDataConnection(int nCmd)
{
    ...
    if (DataSock != INVALID_SOCKET &&
        !setsockopt(DataSock, SOL_SOCKET, (nCmd == CMD_STOR) ? SO_RCVBUF : SO_SNDBUF, (char*)&iBufSize, sizeof(int)))
    {
                            ...
                            do {
                                ...
                                if (pTransfer->zStream.avail_out == 0 || nRet == Z_STREAM_END) {
                                    if (!pClient->SafeWrite(
                                            hFile, pOutBuffer, uiBufferSize - pTransfer->zStream.avail_out))
                                    {
                                        len = -1;
                                        break; // write error
                                    }
...
#endif
{
    ...
    if (hFile != -1) {
        ...
        {
            while (pClient->DataSock != INVALID_SOCKET && (BlockSize = read(hFile, pBuffer, uiBufferSize)) > 0) {
#ifdef CFTPSERVER_ENABLE_ZLIB
                if (pTransfer->eDataMode == ZLIB) {
...
int CFtpServer::CClientEntry::GetFileListLine(char*          psLine,
                                              unsigned short mode,
#ifdef __USE_FILE_OFFSET64
                                              __int64 size,
#else
                    ...
                    if (pClient->CurrentTransfer.nCmd == CMD_NLST) {
                        ...
                        if (fi->pszName[0] != '.' || pTransfer->opt_a) {
                            if (!pClient->AddToListBuffer(
                                    pTransfer, fi->pszName, strlen(fi->pszName), pBuffer, &nBufferPos, uiBufferSize))
                                break;
                        }
                    }
                    else if (fi->pszName[0] != '.' || pTransfer->opt_a) {

                        iFileLineLen = pClient->GetFileListLine(
                            psFileLine, fi->mode, fi->size, fi->mtime, fi->pszName, pTransfer->opt_F);

                        if (!pClient->AddToListBuffer(
                                pTransfer, psFileLine, iFileLineLen, pBuffer, &nBufferPos, uiBufferSize))
                            break;
                    }
...
bool CFtpServer::CEnumFileInfo::FindNext()
{
    ...
    if (i) {
        ...
        if (iDirPathLen + iFileNameLen >= MAX_PATH)
            return false;
        sprintf(szFullPath, "%s%s%s", szDirPath, (szDirPath[iDirPathLen - 1] == '/') ? "" : "/", c_file.name);
        pszName = szFullPath + iDirPathLen + ((szDirPath[iDirPathLen - 1] != '/') ? 1 : 0);
    ...
    if (readdir_r(dp, &dir_entry, &dir_entry_result) == 0 && dir_entry_result != NULL) {
        int iDirPathLen  = strlen(szDirPath);
        int iFileNameLen = strlen(dir_entry.d_name);
        if (iDirPathLen + iFileNameLen >= MAX_PATH)
            return false;

        sprintf(szFullPath, "%s%s%s", szDirPath, (szDirPath[iDirPathLen - 1] == '/') ? "" : "/", dir_entry.d_name);
